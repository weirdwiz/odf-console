...
const useScaleSystemFormValidation = (
  ...
  const { formSchema, fieldRequirements } = React.useMemo(() => {
    ...
    const encryptionSchema = getEncryptionFormSchema(t, false);
    ...
    return {
      formSchema: formSchema as ScaleSystemFormSchema,
      fieldRequirements: {
  ...
  const resolver = useYupValidationResolver<ScaleSystemFormData>(
    formSchema
  ) as unknown as Resolver<ScaleSystemFormData>;
